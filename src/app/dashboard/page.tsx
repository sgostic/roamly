...
export const metadata = { title: "Dashboard — Roamly" };
...
export default function Dashboard() {
  const myRequests = MOCK_REQUESTS.slice(0, 2).map((r) => ({
    ...r,
    offers_count: MOCK_OFFERS[r.id]?.length ?? 0,
  }));
            ...
            {myOffers.map((o) => (
              ...
              >
                <div className="flex justify-between items-start mb-2 gap-2">
                  ...
                  <span className="text-xs rounded-full px-2 py-1 bg-secondary capitalize">
                    {o.status}
                  </span>
                </div>
                <p className="text-sm text-muted-foreground">
                  For: {o.destination} • {format(new Date(o.date_start), "MMM d")}
                </p>
