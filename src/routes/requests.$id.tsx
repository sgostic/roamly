  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-10 max-w-5xl">
        <button onClick={() => nav({ to: "/requests" })} className="text-sm text-muted-foreground hover:text-foreground mb-4 inline-flex items-center gap-1">
        ...
        <div className="rounded-2xl border bg-card p-6 mb-8">
          <div className="flex items-start justify-between flex-wrap gap-3">
            <div>
              <div className="text-sm text-muted-foreground flex items-center gap-1.5">
                <MapPin className="h-3.5 w-3.5" />{req.flexible_destination ? "Flexible destination" : req.destination}
              </div>
              <h1 className="font-display text-3xl font-semibold mt-1">
                {req.flexible_destination ? "Open to suggestions" : req.destination}
              </h1>
            </div>
            <span className={`text-xs rounded-full px-3 py-1 ${accepted || req.status !== "open" ? "bg-muted text-muted-foreground" : "bg-success/15 text-success"}`}>
          ...
          <div className="mt-6 flex gap-2">
            <Link to="/offers/new/$reqId" params={{ reqId: req.id }}>
        ...
        <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
          ...
          {compareIds.length >= 2 && (
            <span className="text-sm text-muted-foreground flex items-center gap-1.5"><Scale className="h-4 w-4" />Comparing {compareIds.length}</span>
          )}
        </div>

        {offers.length === 0 ? (
          <div className="rounded-2xl border bg-card p-10 text-center text-muted-foreground">No offers yet — be the first.</div>
        ) : (
          <div className="grid md:grid-cols-2 gap-4">
            {offers.map((o) => (
              <div key={o.id} className={`rounded-2xl border bg-card overflow-hidden transition-all ${compareIds.includes(o.id) ? "ring-2 ring-primary" : ""} ${o.status === "rejected" ? "opacity-60" : ""}`}>
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">by {o.provider_company || o.provider_name}</p>
                    </div>
                    <span className={`text-xs rounded-full px-2 py-1 whitespace-nowrap ${o.status === "accepted" ? "bg-success/15 text-success" : o.status === "rejected" ? "bg-destructive/15 text-destructive" : "bg-secondary"}`}>{o.status}</span>
                  </div>
                  <p className="text-2xl font-display font-semibold text-primary">{o.currency} {o.price_total.toLocaleString()}</p>
                  ...
                  {o.description && <p className="text-sm mt-3 line-clamp-3 text-muted-foreground">{o.description}</p>}
            ...
            <table className="w-full text-sm min-w-[600px]">
              <thead>
                <tr className="border-b">
                  ...
                  {compared.map((o) => <th key={o.id} className="text-left py-2 px-3 font-display font-semibold">{o.title}</th>)}
                </tr>
              </thead>
              <tbody>
                <tr className="border-b"><td className="py-3 text-muted-foreground">Price</td>{compared.map((o) => <td key={o.id} className="py-3 px-3 font-semibold text-primary">{o.currency} {o.price_total.toLocaleString()}</td>)}</tr>
                <tr className="border-b"><td className="py-3 text-muted-foreground">Accommodation</td>{compared.map((o) => <td key={o.id} className="py-3 px-3">{o.accommodation || "—"}</td>)}</tr>
                <tr className="border-b"><td className="py-3 text-muted-foreground">Included</td>{compared.map((o) => <td key={o.id} className="py-3 px-3">{o.included_services.join(", ") || "—"}</td>)}</tr>
                <tr><td className="py-3 text-muted-foreground">Provider</td>{compared.map((o) => <td key={o.id} className="py-3 px-3">{o.provider_company || o.provider_name}</td>)}</tr>
              </tbody>
            </table>
          </div>
        )}
