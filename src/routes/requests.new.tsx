...
export const Route = createFileRoute("/requests/new")({
  head: () => ({ meta: [{ title: "Post a trip request — Roamly" }] }),
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-10 max-w-2xl">
        ...
        <p className="text-muted-foreground mb-8">Tell providers what you're looking for. They'll send personalized offers.</p>
        <form onSubmit={submit} className="space-y-6 rounded-2xl border bg-card p-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><Label>From</Label><Input type="date" required value={dateStart} onChange={(e)=>setDateStart(e.target.value)} /></div>
            <div><Label>To</Label><Input type="date" required value={dateEnd} onChange={(e)=>setDateEnd(e.target.value)} /></div>
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div><Label>Travelers</Label><Input type="number" min={1} value={travelers} onChange={(e)=>setTravelers(+e.target.value)} /></div>
            <div><Label>Budget min ($)</Label><Input type="number" min={0} value={budgetMin} onChange={(e)=>setBudgetMin(+e.target.value)} /></div>
            <div><Label>Budget max ($)</Label><Input type="number" min={0} value={budgetMax} onChange={(e)=>setBudgetMax(+e.target.value)} /></div>
          </div>
          <div>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {PREFERENCES.map((p) => (
                <button type="button" key={p} onClick={()=>togglePref(p)}
                  className={`text-sm rounded-full px-3 py-1.5 border transition-colors ${prefs.includes(p) ? "bg-primary text-primary-foreground border-primary" : "bg-background hover:bg-secondary"}`}>
                  {p}
                </button>
              ))}
            </div>
